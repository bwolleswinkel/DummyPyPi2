...
def test_set_algo_options():
    ...
    assert dp.config.RTOL == 1E-5 and dp.config.ATOL == 1E-8, "RTOL and ATOL should be set to 1E-5 and 1E-8 by default, respectively"
    ...
    assert dp.config.RTOL == 1E-5 and dp.config.ATOL == 1E-3, "RTOL and ATOL should be set by the global setter to 1E-5 and 1E-3, respectively"
...
def test_algo_options_context_manager():
    ...
    if not (dp.config.RTOL == 1E-5 and dp.config.ATOL == 1E-8):
